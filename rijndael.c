/*
 *  Devano Jose
 *  D24126257
 */

#include <stdlib.h>
#include "rijndael.h"
#define BLOCK_SIZE 16
#include <string.h>


// Declarations for mixed columns
unsigned char gmul(unsigned char a, unsigned char b);
unsigned char xtime(unsigned char x);

// Fixed numbers for the expand key  
static const unsigned char rcon[11] = {
  0x00, 0x01, 0x02, 0x04,
  0x08, 0x10, 0x20, 0x40,
  0x80, 0x1B, 0x36
};

// Can't use any outside libraries from aes.py
static const unsigned char sbox[256] = {
  ,
};
static const unsigned char inv_sbox[256] = {
  
};


// For Encryption, replaces value for one in s_box
void sub_bytes(unsigned char *block) {
  for (int i = 0; i < 16; i++) {
    block[i] = sbox[block[i]];
  }
}


// 4x4 move each row by that number
void shift_rows(unsigned char *block) {
  unsigned char temp;

  //Move left by 1
  temp = block[1];
  block[1]  = block[5];
  block[5]  = block[9];
  block[9]  = block[13];
  block[13] = temp;

  //Move left by 2
  temp = block[2];
  block[2]  = block[10];
  block[10] = temp;
  temp = block[6];
  block[6]  = block[14];
  block[14] = temp;

  //Move left by 3 
  temp = block[15];
  block[15] = block[11];
  block[11] = block[7];
  block[7]  = block[3];
  block[3]  = temp;
}

// Mix Column
void mix_columns(unsigned char *block) {
  for (int i = 0; i < 4; i++) {
    int col = i * 4;

    unsigned char a0 = block[col];
    unsigned char a1 = block[col + 1];
    unsigned char a2 = block[col + 2];
    unsigned char a3 = block[col + 3];

    block[col]     = gmul(a0, 2) ^ gmul(a1, 3) ^ a2 ^ a3;
    block[col + 1] = a0 ^ gmul(a1, 2) ^ gmul(a2, 3) ^ a3;
    block[col + 2] = a0 ^ a1 ^ gmul(a2, 2) ^ gmul(a3, 3);
    block[col + 3] = gmul(a0, 3) ^ a1 ^ a2 ^ gmul(a3, 2);
  }
}

unsigned char xtime(unsigned char x) {
  return (x << 1) ^ ((x >> 7) * 0x1B);
}
unsigned char gmul(unsigned char a, unsigned char b) {
  unsigned char p = 0;
  while (b) {
      if (b & 1)
          p ^= a;
      a = xtime(a);
      b >>= 1;
  }
  return p;
}

// For decryption
void invert_sub_bytes(unsigned char *block) {
  for (int i = 0; i < BLOCK_SIZE; i++) {
    block[i] = inv_sbox[block[i]];
  }
}

void invert_shift_rows(unsigned char *block) {
  unsigned char temp;

    //Move right by 1
    temp = block[13];
    block[13] = block[9];
    block[9]  = block[5];
    block[5]  = block[1];
    block[1]  = temp;

    //Move right by 2
    temp = block[2];
    block[2] = block[10];
    block[10] = temp;
    temp = block[6];
    block[6] = block[14];
    block[14] = temp;

    //Move right by 3 
    temp = block[3];
    block[3]  = block[7];
    block[7]  = block[11];
    block[11] = block[15];
    block[15] = temp;
}

void invert_mix_columns(unsigned char *block) {
  for (int i = 0; i < 4; i++) {
    int col = i * 4;

    unsigned char a0 = block[col];
    unsigned char a1 = block[col + 1];
    unsigned char a2 = block[col + 2];
    unsigned char a3 = block[col + 3];

    block[col]     = gmul(a0, 0x0e) ^ gmul(a1, 0x0b) ^ gmul(a2, 0x0d) ^ gmul(a3, 0x09);
    block[col + 1] = gmul(a0, 0x09) ^ gmul(a1, 0x0e) ^ gmul(a2, 0x0b) ^ gmul(a3, 0x0d);
    block[col + 2] = gmul(a0, 0x0d) ^ gmul(a1, 0x09) ^ gmul(a2, 0x0e) ^ gmul(a3, 0x0b);
    block[col + 3] = gmul(a0, 0x0b) ^ gmul(a1, 0x0d) ^ gmul(a2, 0x09) ^ gmul(a3, 0x0e);
}
}

// Shared Operation
void add_round_key(unsigned char *block, unsigned char *round_key) {
  for (int i = 0; i < BLOCK_SIZE; i++) {
    block[i] ^= round_key[i];
}
}


unsigned char *expand_key(unsigned char *cipher_key) {
  // 11 round keys * 16 bytes
  unsigned char *expanded_keys = malloc(176);  
  int bytes_generated = 16;
  int rcon_iter = 1;
  unsigned char temp[4];

  // Copy
  for (int i = 0; i < 16; i++) {
      expanded_keys[i] = cipher_key[i];
  }

  while (bytes_generated < 176) {
      // Last 4 bytes for key block
      for (int i = 0; i < 4; i++) {
          temp[i] = expanded_keys[bytes_generated - 4 + i];
      }

      // 16 bytes key schedule 
      if (bytes_generated % 16 == 0) {
          // Rotate
          unsigned char t = temp[0];
          temp[0] = temp[1];
          temp[1] = temp[2];
          temp[2] = temp[3];
          temp[3] = t;

          // Sub using S-box
          for (int i = 0; i < 4; i++) {
              temp[i] = sbox[temp[i]];
          }

          // RCON
          temp[0] ^= rcon[rcon_iter++];
      }

      // XOR with 16 bytes 
      for (int i = 0; i < 4; i++) {
          expanded_keys[bytes_generated] = expanded_keys[bytes_generated - 16] ^ temp[i];
          bytes_generated++;
      }
  }

  return expanded_keys;;
}

unsigned char *aes_encrypt_block(unsigned char *plaintext, unsigned char *key) {
  unsigned char *output =
      (unsigned char *)malloc(sizeof(unsigned char) * BLOCK_SIZE);

    memcpy(output, plaintext, BLOCK_SIZE);  
    
    unsigned char *round_keys = expand_key(key);
    add_round_key(output, round_keys); // First 16 bytes 
  
    for (int round = 1; round <= 9; round++) {
      sub_bytes(output);
      shift_rows(output);
      mix_columns(output);
      add_round_key(output, round_keys + (round * BLOCK_SIZE));
    }

    sub_bytes(output);
    shift_rows(output);
    add_round_key(output, round_keys + (10 * BLOCK_SIZE));

    free(round_keys);
    return output;
}

unsigned char *aes_decrypt_block(unsigned char *ciphertext,
                                 unsigned char *key) {

  unsigned char *output =
      (unsigned char *)malloc(sizeof(unsigned char) * BLOCK_SIZE);

  memcpy(output, ciphertext, BLOCK_SIZE);

  unsigned char *round_keys = expand_key(key);
  add_round_key(output, round_keys + (10 * BLOCK_SIZE));

  for (int round = 9; round >= 1; round--) {
    invert_shift_rows(output);
    invert_sub_bytes(output);
    add_round_key(output, round_keys + (round * BLOCK_SIZE));
    invert_mix_columns(output);
  }

  invert_shift_rows(output);
  invert_sub_bytes(output);
  add_round_key(output, round_keys);


  free(round_keys);
  return output;
}
